...
pub mod p2 {
    const DIGIT_WORDS: [&str; 18]  = ["1", "2", "3", "4", "5", "6", "7", "8", "9", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"];

    /// Concat the first and last digit, or spelled digit, of each line, parse the number and sum them all
    pub fn solve(input: &str) -> usize {
        input.lines().map(|line| {
            let first = (0..DIGIT_WORDS.len()).filter_map(|dwi| {
                line.find(DIGIT_WORDS[dwi]).map(|i| (i, dwi % 9))
            }).min_by_key(|fws| fws.0).unwrap().1 + 1;

            let last = (0..DIGIT_WORDS.len()).filter_map(|dwi| {
                line.rfind(DIGIT_WORDS[dwi]).map(|i| (i, dwi % 9))
            }).max_by_key(|fws| fws.0).unwrap().1 + 1;
